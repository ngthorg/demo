...
import promiseMiddleware from '../universal/utils/promiseMiddleware';
...
async function render(req, res, next) {
  ...
  match(
    { routes: routers(), location: req.url },
    async (err, redirectLocation, routerState) => {
      try {
        ...
        const prepareRouteMethods = routerState.components.map(
        ...
        const html = renderToString(
          <HtmlComponent
            markup={body}
            state={serialize(JSON.stringify(initialState))}
          />,
        );
